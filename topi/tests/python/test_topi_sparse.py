...
def verify_sparse_length_sum(M, K, Lsize, Lmax):
    def get_ref_data():
    ...
    sls_np = sparse_length_sum_ref(d_np, i_np, l_np)
    ...
    SLS = topi.sparse.sparse_length_sum(D, I, L)
...
def verify_sparse_length_sum_fused_8_bit_rowwise(M, K, Lsize, Lmax):
    def get_ref_data():
    ...
    def sparse_length_sum_fused_8_bit_rowwise_ref(D, I, L):
    ...
    d_np, l_np, i_np = get_ref_data()
    sls_np = sparse_length_sum_fused_8_bit_rowwise_ref(d_np, i_np, l_np)

    D = tvm.placeholder(d_np.shape, dtype="uint8", name="D")
    I = tvm.placeholder((i_np.shape), dtype="int32", name="I")
    L = tvm.placeholder(l_np.shape, dtype="int32", name="L")

    SLS = topi.sparse.sparse_length_sum_fused_8_bit_rowwise(D, I, L)
    s = tvm.create_schedule(SLS.op)
    f = tvm.build(s, [D, I, L, SLS], target="llvm")
    d = tvm.ndarray.array(d_np)
    i = tvm.ndarray.array(i_np)
...
def test_sparse_length_sum():
    verify_sparse_length_sum(M=10, K=16, Lsize=40, Lmax=20)
    verify_sparse_length_sum(M=10, K=16, Lsize=40, Lmax=100)
    verify_sparse_length_sum(M=1, K=1, Lsize=4, Lmax=100)

def test_sparse_length_sum_fused_8_bit_rowwise():
    verify_sparse_length_sum_fused_8_bit_rowwise(M=10, K=16, Lsize=40, Lmax=20)
    verify_sparse_length_sum_fused_8_bit_rowwise(M=10, K=16, Lsize=40, Lmax=100)
    verify_sparse_length_sum_fused_8_bit_rowwise(M=1, K=12, Lsize=4, Lmax=100)
